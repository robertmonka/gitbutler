...
describe.concurrent("DefaultforgeFactory", () => {
	...
	test("Create Gitea service from detected forge provider", async () => {
		const factory = new DefaultForgeFactory({
			gitHubClient,
			gitHubApi,
			backendApi,
		...
		expect(
			factory.build({
				repo: {
					domain: "code.internal.example",
					name: "test-repo",
					owner: "test-owner",
				},
				baseBranch: "some-base",
				detectedForgeProvider: "gitea",
				forgeOverride: undefined,
			}),
		).instanceOf(Gitea);
	});

	test("Create Gitea service when preferred account matches non-gitea remote host", async () => {
		const factory = new DefaultForgeFactory({
			gitHubClient,
			gitHubApi,
			backendApi,
	...
	test("Project Gitea account matching repository host takes precedence over detected provider", async () => {
		const factory = new DefaultForgeFactory({
			gitHubClient,
			gitHubApi,
			backendApi,
